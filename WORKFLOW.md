...
**Rule:** Every substantive entry in a brain file carries a small YAML metadata block: `added:`, `last_verified:`, `source:`. The weekly auto-audit uses this to detect stale content mechanically.
...
### Where applied
- STATUS.md blockers (each line)
- PROJECT_BRAIN.md CRITICAL RULES table (source column)
- DECISIONS.md (last_verified when re-checked)
- TECH_STACK.md architecture facts
- MARKET_GUIDE.md banners

### Staleness triggers (weekly audit)
- `last_verified` > 60 days → WARNING
- `last_verified` > 180 days → VIOLATION
- No metadata → MISSING_METADATA warning

Full spec: `SnapAI_Brain_Files_Management_Plan_2026-07-05.md` Part 6 Rule 4.

---

## 17. Session Learnings Log — SESSION_LOG_* archetype (added 2026-07-06 — brain files cleanup Phase 3)

**Rule:** Substantive sessions with retrospective content write a `SESSION_LOG_YYYY-MM-DD_<topic>.md` in `ScopeSnapAI/session_logs/`. Format: metadata header, context, goal, what tried, what worked, what DIDN'T work, root causes, resolution, lessons, follow-ups, references, change log.

### When to create
- Substantive debug/fix (multiple hypotheses tried)
...
- Pure discussion / strategic planning (goes in ACTIVE_TASKS block)

### Session-end update ritual (analyze + propose, do NOT ask questions)

When Shoab says *"update the brain files"* or at end of substantive session:
1. AI analyzes what happened against Section 7 routing table in `SnapAI_Project_Instructions.md`.
2. AI presents ONE consolidated proposal: *"I'll update these files with these changes: [file 1] — [summary]. [file 2] — [summary]. ... Say yes to write all, or flag exceptions."*
3. Shoab replies "yes" or flags specific items.
4. AI executes in one pass. Reports result.

**Do NOT ask a series of yes/no questions.** The AI already knows what each file is for. Analyze once, propose once, execute once.

Full spec + retroactive candidates: `SnapAI_Brain_Files_Management_Plan_2026-07-05.md` Part 7 + `session_logs/SESSION_LOG_2026-07-06_brain_files_cleanup.md`.
