...
| Brand voice on production landing pages NOT yet updated | `snapai-copywriting` skill has the new "builders who spent 18 months in supply houses, Mark Delgado on the input panel" voice. Production homepage and `/tech` page still use older SaaS template copy ("Photograph any HVAC unit. SnapAI identifies the equipment", "1 in 4 Houston HVAC techs is senior-level", "Three taps. Professional estimate."). Marketing skill exists; landing pages not yet rewritten with it. | open — pre-marketing-launch fix |
| Tier naming is 3-way inconsistent across live UI surfaces | `/homeowner` landing promises "Good / Better / Best" with Best = "FULL REPLACEMENT — new system install". `/tech` landing also says "Good / Better / Best". Estimate Builder shows "Option A / Option B (★ REC) / Option C" with descriptive labels "Fix Today / Fix + Prevent Next Failure / Consider Replacing". `/d/` share URL uses none of these. Promise mismatch: `/homeowner` Best = "full replacement", actual Option C ≠ replacement (expanded leak repair). DEC-049 documented 2-way; live is 3-way. | DEC-049, open |
