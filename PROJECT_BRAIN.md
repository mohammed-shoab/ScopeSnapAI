...
|------|---------|---------|------------|------|
| 2026-09-17 (visual) | Houston (staging only) | 3 NEW findings - found ONLY by looking | First visual click-through of the audit: Playwright drove US staging and captured 9 screenshots, which were then read. F11 FIXED (PR merged 57300dc): share_url was HARDCODED to the prod domains, so every staging share link pointed at prod where the token 404s - now derived from settings.frontend_url with PK host mapping. F13 FIXED: reading receipt rendered '115-141 WITHIN RANGE' while the classifier treats 141 as HIGH - low_threshold is inclusive but high_threshold is EXCLUSIVE; band top now steps back one unit (115-140, 225-275 per canonical table). F12 OPEN (needs a product decision): header pill says 'High Confidence' while the receipt 4 lines below says 'Confidence: MEDIUM' - two different backend fields, both labelled 'Confidence'. 14 tests, red-green (10 fail unpatched), suite 212 green. Dashboard/pricing/nav render clean; US pricing correctly shows $. | 57300dc |
| 2026-09-18 (PM) | Houston + PK (**PROD**) | **PROMOTED TO PROD** - 9 fixes live, 4 CRITICAL CVEs cleared | `f2ba07e -> 18a1c3e` DEC-070 scoped overlay of 24 files (NOT a merge). Main CI all green; Railway prod ACTIVE + deployment successful (proves alembic resolved 048 - the latent 'Can't locate revision 048' boot failure is CLOSED); both Vercel projects green. **Dependabot 28 -> 9, all 4 CRITICAL cleared** (next 16.2.12 -> 16.3.4); remaining 9 (js-yaml x2, brace-expansion x4, fflate, weasyprint x2) are equally behind on staging, not regressions. PROD HAND-VERIFIED IN CHROME: full US walk to Refrigerant Leak + estimate + PDF + homeowner report. **F14 live** (receipt reads prose; API `target_source: null` = fail-closed working). **F13 live** (copy reads 115-140). **CP7 PASS on prod** - 1342-byte body byte-identical under forged X-Market incl. SQLi string (first attempt was a VACUOUS 33-byte 404 on the wrong token - caught). F12 still present, now proven at data layer (high vs Medium in one payload). NEW: F15 PK Urdu bidi displaces '90' + untranslated 'LAST 5 ASSESSMENTS' (first-ever Urdu visual confirmation); F16 prod-only duplicated pricing_rules (28 vs 14) AND all rows deprecated=true yet rendered as National Defaults (both envs); F17 React #418 hydration on public report; F18 report title 'HVAC Report - - Company'. **PKR/F6 UNVERIFIABLE: zero PK companies exist in either env (prod 17 US, staging 4 US).** CORRECTION: 'Recent Assessments empty' was staging DATA, not a bug - prod renders populated. Prod DB: alembic 048, RLS 56 tables, trigger `rls_auto_enable_trg` armed. | 18a1c3e |
| 2026-09-18 | Houston + PK (staging) + PROD db | AUDIT CLOSED-OUT - 9 of 14 findings fixed | F14 raw DB identifiers leaking into the reading receipt FIXED - backend sanitisers, FAIL CLOSED, 25 tests, suite **237 green**. Found ONLY by a real-browser click-through; 212 tests, ZAP active (140 rules), semgrep, gitleaks and k6 were all green on it. PROD: migration 048 applied + verified (tables_rls_on=10, trigger_armed=1, evtenabled='O', anon_can_exec=false, alembic=048) - **CP2 CLOSED**. CP3 CLOSED as ACCEPTED (DEC-137) - CRON_SECRET stays prod-only by Shoab's decision. OPEN: F12 confidence contradiction (product decision), receipt shows no target numbers, empty Recent Assessments, Gemini key exposure accepted un-rotated. PROMOTE SCOPED NOT RUN: git merge gives 11 conflicts (overlay-based history); every main-only line in the 3 conflicted code files is pre-fix code - use scripts/promote-to-prod.sh. Prod CODE still f2ba07e. | 8a7d084 |
| 2026-09-17 | Houston + PK (staging only) | AUDIT CLOSED-OUT - 6 of 10 findings fixed | F9 non-UUID path param -> unhandled 500 FIXED (PR #70, 7 params typed UUID + narrow DBAPIError handler, 18 tests, 198 suite green). Assessment->diagnosis walkthrough now REAL (PR #71): /assessments/new -> /assess -> manual Step Zero -> Not Cooling -> question tree -> RESOLVED 'Ductwork Leak | High Confidence', 0x 5xx. Phase 4: CP1 PASS, CP2 FAIL (prod alembic 047 vs staging 048 - DEC-135 migration missing on prod), CP3 FAIL (CRON_SECRET in prod only), CP4 PASS, CP5 PASS (NOVEL=0, staging 26 < prod 33 console errors), CP6 not meaningful (ZAP/k6 poisoned the window), CP7 PASS (forged X-Market returns byte-identical result - live proof of F7). CORRECTIONS: 'Step Zero is a hard photo gate' was WRONG - a manual-entry tab exists. F6 PKR fix still NOT visually confirmed - pk-staging authenticated nav times out. | a8ffa3a |
| 2026-09-16 (later) | Houston + PK (staging only) | FIXES SHIPPED - 4 of 8 findings closed | F6 PKR-in-USD + F8 ambient-floor (PR #66), F2 SSRF-via-redirect + F7 spoofable-market (PR #67). 25 regression tests added; pytest 180 pass. Authenticated audit/ harness PASSED for the first time (AUTH OK, but lands on /onboarding not /dashboard - deeper flows still unexercised). PHASE 4 CHECKPOINT 2 FAILS: staging alembic 048 vs prod 047 - prod is MISSING the DEC-135 RLS migration (c3eb21c not an ancestor of main). F3 re-diagnosed: not a code defect (k6 hit /api/health which touches no tables) but advisors found 9 RLS policies re-evaluating auth.<fn>() per row + 17 unindexed FKs, which DO bite under authenticated load. F1 Gemini key remains open by Shoab's decision. | f1ad5cd |
| 2026-09-16 | Houston + PK (staging only) | COMPLETE - 8 findings | Zero fixed in-loop (audit was read-only apart from fast-uri). snapai-full-audit mode=full. Pre-flight 6/7 (semgrep installed mid-run; P7 not enumerated). Railway $0.43/$15. pytest 155 pass. gitleaks 31 findings (1 real: live Gemini key in public history, Shoab accepted risk). semgrep 32 findings (28 avoid-sqlalchemy-text FALSE POSITIVES - identifiers come from frozen MarketTables, user values bound-param). ZAP passive 0 FAIL/11 WARN/56 PASS. ZAP active 0 FAIL/1 WARN/140 PASS (SQLi, RCE, SSTI, XXE, cloud-metadata all pass). k6 50VU PASS / 200VU p95 FAIL / 500VU both FAIL. PSI assertions R-410A 130->ok, R-22 88, R-32 140 all PASS. Urdu 653 runs 0 tofu PASS. PKR formatting FAIL (F6). Cross-market split (F7). Ambient-floor fallback (F8). | 556414c |
