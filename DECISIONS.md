...
## DEC-130 — Legal-safe wordings v1 SHIPPED to prod: authenticated-vs-public surface taxonomy, contractor gate C3, ToS, disclaimers, single landing page (2026-07-06)

Shipped the "legal-safe wordings v1" change set to prod (main `e22580a`, from staging `19563f3`). Reduces consumer-protection / DTPA / FTC exposure by making the licensed contractor — never SnapAI — the party that "diagnoses" on all public + homeowner surfaces. Umbrella decision; sub-decisions:

- **Surface taxonomy (core rule):** the dividing line is AUTHENTICATED vs PUBLIC. Authenticated contractor app (login + signed ToS + verified contractor) may say "diagnose" (ToS Sec 2A-2C protects it) — contractor-facing "diagnose" UI KEPT unchanged. Public surfaces (the single /tech landing page) and homeowner outputs (report, PDF, share link, emails) get consumer rules: no "diagnosis" by SnapAI, tech/contractor is always the grammatical subject, all conclusions attributed to the licensed contractor.
- **Contractor GATE C3 (the load-bearing piece):** onboarding now REQUIRES a license number + two attestation checkboxes (licensed-contractor + Sec-2A-2C decision-support acknowledgment linking to /tos); Skip removed. Enforced in backend `PATCH /api/auth/me/company` (422 on blank license) + `app/(app)/layout.tsx` guard (redirects incomplete profiles to /onboarding). New columns `attestation_accepted_at` + `terms_ack_version` via migration **045** (prod alembic head = 045, verified). The gate is the prerequisite that makes the whole definitional-clause strategy real.
- **ToS at /tos:** full 16-section contractor ToS (framework DRAFT), incl. Sec 2 decision-support definitional clause + Sec 7 indemnification. Ships labeled **"Draft pending final legal review (Alfred) — Not yet effective"** with entity "operated by Mainnov". Footer Terms link on public pages. NOT YET EFFECTIVE until real Texas counsel signs (Gate 2).
- **Disclaimers live:** Layer-1 (public /tech landing), Layer-4 (always-visible in-app FaultResolutionScreen — auth + public /d/ share), Layer-5 (homeowner web report /r AND PDF contractor_estimate.html). Layer-5 on the web report was a prod-QA catch (was PDF-only; added).
- **Single public landing page:** `/` and `/homeowner` 301/308-redirect to `/tech` (`permanent: true` = 308; kept deliberately — SEO-equivalent for retired GET pages, Next-idiomatic); old `app/page.tsx` + `app/homeowner/page.tsx` deleted. /tech is the ONLY public landing page.
- **/methodology PUBLISHED:** confidence bands defined numerically from real code (report score: High >=90 / Good 75-89 / Fair 60-74 / Low <60; in-app 3-tier from cascade: sensor high >=85%, OCR low <50%). NO accuracy/false-positive figure stated (FTC substantiation — parked for Will's file).
- **Geo-neutral public copy:** all "Houston"/city names removed from public surfaces ("independent HVAC shops"); geo stays backend-only (reaffirms MARKET_GUIDE rule).
- **"honest"/"no upsell" PARKED (Gate 1):** every SnapAI self-claim of "honest recommendation" removed (incl. a root `app/layout.tsx` meta-description miss caught in prod QA); does not ship until Will's algo-bias substantiation file proves it.
- **D7 cascade field rename:** `confirmed_fault -> suggested_finding_for_review` (+ 2 fields) across prompt AND reader `ai_cascade.py` (closed the silent-fallback `.get()` trap); guard test added.
- **Pre-commit hook (D8):** `.pre-commit-config.yaml` + `scripts/check_legal_banned_strings.py --staged` blocks new "diagnos*"/DEC-088 bans/city names in homeowner+public paths (diff-mode; /tos excluded as counsel-controlled).
- **Promotion method:** main was 43 commits ahead of staging on 11 shared files, so a scoped file-overlay would have reverted prod work. Used a 3-way apply of the deployment diff onto main (verified main's independent changes preserved, e.g. tech/page.tsx). Record this as the safe promote pattern when main has diverged.

**Cross-refs:** DEC-088 (no future-tense homeowner promises), DEC-070 (staging-first), DEC-111 (verify code state), DEC-004 (sandbox clone), Alfred Gate-2 review (DTPA §17.42, FTC AI-washing, FAA class-waiver). **Open follow-ups:** real Texas counsel sign-off to make ToS effective + remove draft banner; Will's substantiation file to unpark "no upsell"; TDLR license verification job; Privacy Policy multi-state review (H4).
