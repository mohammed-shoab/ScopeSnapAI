...
export default function StepZeroPanel({ assessmentId, clerkToken, onConfirm, onSkip }: Props) {
  ...
  const runOCR = useCallback(async () => {
    ...
    let finalTier = 4; // default to manual
    ...
    try {
      ...
      // Gemini returns a single unit-level confidence (0–100).
      // Derive per-field confidence: null fields → 0; non-null fields inherit overall.
      // ≥70 → trust as-is | 40–69 → yellow border (needs confirmation) | <40 → treat as missing
      ...
      for (const field of fieldsToCheck) {
        ...
        if (fieldConf < 40) {
          // Missing / very low confidence — Tier 3 DB lookup will try to fill
        } else if (fieldConf < 70) {
          needsConfirm.add(field);
        }
      }

      // ── Tier 3: DB lookup fills missing low-confidence fields ──────────────
      // Tonnage is the key index — if we have it, look up electrical specs
      if (result.outdoor.tonnage !== null && overallConfidence >= 40) {
        ...
        if (elecSpec) {
          ...
          const patchedResult = { ...withSource, outdoor: patched };
          setOcrResult(patchedResult);
      ...
      // Pre-fill manual unit with any partial data if available
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
          ...
          <div className="p-3 grid grid-cols-2 gap-2">
            {OCR_FIELDS.map(({ key, label, unit, type, badge }) => {
              ...
              const isEstField = badge === "est";
              // BUG-034: yellow border for 40-69 confidence fields (needs confirmation)
