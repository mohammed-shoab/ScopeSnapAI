...
function AssessPageInner() {
  ...
  const handleComplaintSelected = async (complaintId: ComplaintId) => {
    ...
    try {
      ...
      if (!r.ok) {
        const detail = await r.json().then((d: { detail?: string }) => d.detail).catch(() => "Failed to create assessment");
  ...
  // ── WS-L3: Service checklist complete → service estimate ─────────────────────
  const handleServiceComplete = useCallback(async (result: ServiceEstimateResult, sessionId: string) => {
    setServiceEstimate(result);
    setDiagnosedSessionId(sessionId);
    setPhase("estimating");
    setError(null);
    try {
      const headers = await getAuthHeaders();
      // For service, we POST to /api/estimates/fault-card with card_id=2 (Service)
      // but use the service estimate result's total_typical as the base.
      // The assessment page will display the service findings.
      // We navigate to the assessment with a service=1 param so it can render findings.
      router.push(`/assessment/${assessmentId}?service=1&session=${sessionId}`);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Error completing service");
      setPhase("diagnostic");
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [assessmentId, router]);
  ...
  // ── diagnostic: question-tree flow (or service checklist for Tab S) ─────────
  if (phase === "diagnostic" && assessmentId && complaintType) {
    const complaintLabel = COMPLAINT_OPTIONS.find(o => o.id === complaintType)?.label ?? complaintType;
    return (
