...
## Housekeeping batch shipped to prod — 2026-06-24/26 (commit 1a8fe5c)

Closed out the post-audit housekeeping. All staging-verified (e2e #58/#59 green) then promoted to prod (file-level, DEC-070); prod healthy + app renders.

- **report_short_id hardening (non-breaking):** added `@limiter.limit("30/minute")` to the public `correct-age` route (was the only un-throttled enumeration oracle); aligned the 4-digit short-id generators in `estimates.py` + `diagnostic.py` to **6 digits** (matches `fault_estimate.py`; BUG-030b). Short-id resolver + the **2027-05-19 sunset** (DEC-016) are unchanged — product decision: keep the sunset (old pre-cutover short links 404 after that date).
- **Sentry noise filter:** `sentry.client.config.ts` now drops `Object Not Found Matching Id…` (CefSharp engine behind Outlook "Safe Links" URL scanning — bot noise, not app code) via `ignoreErrors` + `beforeSend`.
- **Dependabot bumps merged + smoke-tested (built green on Linux + e2e):** pillow 10.4->12.2, weasyprint 62.3->68.0 (note: **weasyprint is dead code** — PDFs use a pure-Python writer; safe to remove the dep later), numpy ->1.26.4, xgboost ->2.1.4, openpyxl ->3.1.5, pydantic-settings 2.14.2; frontend npm-minor group (clerk 7.5.7, posthog-js, @sentry/nextjs, @types/google.maps). #20's previously-red e2e was a stale-branch artifact — green once merged onto staging (which has the test-harness fix).
- **Hydration (React #418):** the report-page QR mismatch was already fixed (a26e10e). Verified the other flagged spots are NOT bugs: StepZeroPanel `Math.random` is inside a `useEffect`; SendMomentModal `sessionStorage` is in imperative helpers; ReportClient `new Date().getFullYear()` is the negligible NYE-boundary edge (left as-is to avoid an age-flash).
- **Sentry issues:** 3 unresolved are (a) hydration — fixed, mark resolved once events age out; (b) `/tech` UnhandledRejection — now filtered (bot noise); (c) `NEXT16-STG` — synthetic migration smoke-test marker. All safe to close.
- **By-design / no action (confirmed):** CSP `unsafe-inline` (ignored by browsers under strict-dynamic) + `unsafe-eval` (required by Google Maps); k6 capacity ceiling (single Hobby replica; scaling costs money = revisit only at real load).
